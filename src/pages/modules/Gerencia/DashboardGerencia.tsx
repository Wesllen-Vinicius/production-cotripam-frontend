import { motion } from 'framer-motion'
import { NavigationGerencia } from '../../../components/TabsGerencia'
import { useAuth } from '../../context/AuthContext'

export function DashBoardGerencia() {
  const { Logout } = useAuth()

  function handleLogout() {
    Logout()
  }
  return (
    <>
      <div className="grid grid-cols-1 gap-4 pb-4">
        <div>
          <h2 className="text-2xl tracking-tight font-extrabold text-gray-900 dark:text-white">
            PAINEL DE ADMINISTRADOR
          </h2>
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.95 }}
            onClick={handleLogout}
            type="submit"
            className=" group relative  justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md dark:bg-[#e2e8f0] dark:text-black text-white bg-black focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
          >
            Sair
          </motion.button>
        </div>
        <nav className="flex" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1 md:space-x-3">
            <li className="inline-flex items-center">
              <a className="inline-flex items-center text-sm font-medium text-gray-700 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white">
                <svg
                  className="w-4 h-4 mr-2"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"></path>
                </svg>
                Home
              </a>
            </li>
            <li>
              <div className="flex items-center">
                <svg
                  className="w-6 h-6 text-gray-400"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
                    clipRule="evenodd"
                  ></path>
                </svg>
                <a className="ml-1 text-sm font-medium text-gray-700 hover:text-gray-900 md:ml-2 dark:text-gray-400 dark:hover:text-white">
                  Painel de Administrador
                </a>
              </div>
            </li>
          </ol>
        </nav>
      </div>
      <div className="h-auto">
        <NavigationGerencia />
      </div>
    </>
  )
}
