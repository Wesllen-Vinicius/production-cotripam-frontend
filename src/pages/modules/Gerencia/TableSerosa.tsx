...
export function TableSerosa() {
  const [serosas, setSerosas] = useState<Serosas[]>([])
  const [value, setValue] = useState(0)
  const [total, setTotal] = useState(0)
  const [page, setPage] = useState(1)

  const deleteSerosa = async (id: number) => {
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          await api.delete(`serosa/${id}`).then(() => {
            setValue((c) => c + 1)
          })
  ...
  const onSearch = (event: FormEvent<HTMLFormElement>) => {
    ...
    api
      .get<{ count: number; termo: string; serosas: Serosas[] }>(
        `serosa?${params.toString()}`
      )
      .then((res) => {
        const serosas = res.data.serosas.map((serosa) => ({
          ...serosa,
          createdAt: new Date(serosa.createdAt),
        }))
        setSerosas(serosas)
  ...
  useEffect(() => {
    api
      .get<{ count: number; serosas: Serosas[] }>(`serosa?page=${page}`)
      .then((res) => {
        const serosas = res.data.serosas.map((serosa) => ({
          ...serosa,
          createdAt: new Date(),
              ...
              return (
                ...
                >
                  ...
                  <td
                    ...
                    className="text-center px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap"
                  >
                    {serosa.User.Comarca.nome}
                  </td>
                  <td
                    scope="row"
                    className="text-center px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap"
                  >
                    {serosa.User.nomeUser}
                  </td>
                  <td
                    scope="row"
                    className="text-center px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap"
                  ...
                  <td
                    ...
                    className="text-center px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap"
                  >
                    {formateDate(serosa.createdAt)}
                  </td>
                  <td
                    scope="row"
                    className="text-center px-2 py-4 font-medium text-gray-900 dark:text-white whitespace-nowrap"
                  >
                    <EditSerosa id={serosa.id} />
                    <motion.button
                      whileHover={{ scale: 1.1 }}
                      whileTap={{ scale: 0.95 }}
                      className="pl-2"
                      onClick={() => deleteSerosa(serosa.id)}
                    >
                      <img
